...
```{r}
df_map<-select(clean_acc21, ID, State, Start_Lat, Start_Lng)
head(df_map)
```
...
```{r}
mapview(df_map_DC_sf, xcol = "Start_Lng", ycol = "Start_Lat", grid = FALSE)
```

```{r}
mapview(df_map_DC_sf, map.types = "Stamen.Toner")
```
...
ggplot(clean_acc21, aes(x = Is_severe, y=Temperature.F., fill = Is_severe)) + 
  geom_boxplot() +
  labs(title="Temperature by Severity", x="Severity", y = "Temperature(F)")

ggplot(clean_acc21, aes(x = Is_severe, y=Wind_Chill.F., fill = Is_severe)) + 
  geom_boxplot() +
  labs(title="Wind Chill by Severity", x="Severity", y = "Wind Chill")

ggplot(clean_acc21, aes(x = Is_severe, y=Wind_Speed.mph., fill = Is_severe)) + 
  geom_boxplot() +
  labs(title="Wind Speed by Severity", x="Severity", y = "Wind Speed")

ggplot(clean_acc21, aes(x = Is_severe, y=Humidity..., fill = Is_severe)) + 
  geom_boxplot() +
  labs(title="Humidity by Severity", x="Severity", y = "Humidity")

ggplot(clean_acc21, aes(x = Is_severe, y=Pressure.in., fill = Is_severe)) + 
  geom_boxplot() +
  labs(title="Pressure by Severity", x="Severity", y = "Pressure")

ggplot(clean_acc21, aes(x = Is_severe, y=Visibility.mi., fill = Is_severe)) + 
  geom_boxplot() +
  labs(title="Visibility by Severity", x="Severity", y = "Visibility")

ggplot(clean_acc21, aes(x = Is_severe, y=Precipitation.in., fill = Is_severe)) + 
  geom_boxplot() +
...
```
I took t-test on Severity and Numerical weather variables because 
