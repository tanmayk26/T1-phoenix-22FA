...
chitest
```
To identify if the severity of traffic is dependent on Weather Condition, we took a Chi-Squared test as Weather Condition and Severity are categorical variables.
...
```{r cor between month, severity}
=======
```{r chisq on month and severity}
test_Month <- chisq.test(table(clean_acc21$Severity, clean_acc21$Month))
test_Month
```
