...
clean_acc22 %>% 
    ...
    summarise(percentage = n()/nrow(clean_acc22) *100 )%>%
    arrange(-percentage) %>%
...
acc_road_element_per <- tibble(c("None", "Crossing", "Traffic signal", "Crossing and traffic signal", "Station", "Stop" ), pull(accidents_per_roadelement, percentage), .name_repair = ~ c("road_elements", "percentage"))
...
accidents_per_roadelement<-clean_acc22 %>% 
...
```
```{r}
ggplot(data=Crossing_data, aes(x=Is_severe, y=Crossing)) +
...
```
```{r}
ggplot(data=Stop_data, aes(x=Is_severe, y=Stop)) +
  geom_bar(stat="identity")
```
...
```
```{r}
ggplot(data=Station_data, aes(x=Is_severe, y=Station)) +
  geom_bar(stat="identity")
```
...
```
```{r}
ggplot(data=Traffic_Signal_data, aes(x=Is_severe, y=Traffic_Signal)) +
  geom_bar(stat="identity")
```
