...
test <- data_corr[-dat.d,] # remaining 20% test data
...
```{r}
pop_pred=predict(model_lm,newdata = test)
pop_pred1=data.frame(pop_pred)
head(pop_pred1)
```

```{r}
compare <- cbind (actual=test$popularity, pop_pred)  # combine actual and predicted
